...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='carfusion',
                             help='carfusion dataset name')
    ...
    self.parser.add_argument('--vis_thresh', type=float, default=0.13,
    ...
    # mmd loss
    self.parser.add_argument('--use_mmd', action='store_true',
                    help='Enable MMD Loss for DA')
    self.parser.add_argument('--mmd_weight', type=float, default=0.1,
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 24}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.hm_hp:
        opt.heads.update({'hm_hp':dataset.num_joints})
      if opt.reg_hp_offset:
        opt.heads.update({'hp_offset': dataset.num_joints * 2})
    else:
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'multi_pose': {
        ...
        'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
        'dataset': 'carfusion', 'num_joints': 12,
