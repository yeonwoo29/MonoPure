...
def multi_pose_decode(heat, wh, kps, reg=None, hm_hp=None, hp_offset=None, K=100):
  ...
  num_joints = hp_offset.size(1) // 2 if hp_offset is not None else kps.shape[1] // 2
  ...
  if hm_hp is not None:
      ...
      if hp_offset is not None:
          ...
          for j in range(num_joints):
              ind = hm_inds[:, j, :]  # shape: [batch, K]
              gathered_feat = _transpose_and_gather_feat(hp_offset, ind)  # [batch, K, 2]
              gathered.append(gathered_feat.unsqueeze(1))  # [batch, 1, K, 2]
          hp_offset = torch.cat(gathered, dim=1)  # [batch, num_joints, K, 2]
