...
class Debugger(object):
  ...
  def add_carfusion_bbox(self, bbox, score, img_id='default', color=(255, 50, 200), thickness=2):
    """
    bbox: [x1, y1, x2, y2]
    score: float, confidence score
    """
    x1, y1, x2, y2 = map(int, bbox)
    #x1, y1, x2, y2 = [int(v * self.down_ratio) for v in bbox]
    #cv2.rectangle(self.imgs[img_id], (x1, y1), (x2, y2), color, thickness)
    ...
    cv2.putText(self.imgs[img_id], label, text_origin, font, font_scale, color, thickness_text)

  def add_carfusion_hp(self, points, img_id='default', color=(0, 255, 0), radius=3):
    points = np.array(points, dtype=np.int32).reshape(-1, 2)

    # 점 그리기
    for j in range(points.shape[0]):
        cv2.circle(self.imgs[img_id], (points[j, 0], points[j, 1]), radius, color, -1)
    ...
    for e in edges:
        pt1, pt2 = points[e[0]], points[e[1]]
        cv2.line(self.imgs[img_id], (pt1[0], pt1[1]), (pt2[0], pt2[1]), color, 2)
...
# 결과 이미지 저장
  def save_img(self, imgId='default', path='D:/detection/keypoint/CenterNet/out'):
    cv2.imwrite(os.path.join(path, '{}.png'.format(imgId)), self.imgs[imgId])
    
  def save_all_imgs(self, path='D:/detection/keypoint/CenterNet/out', prefix='', genID=False):
    if genID:
      try:
