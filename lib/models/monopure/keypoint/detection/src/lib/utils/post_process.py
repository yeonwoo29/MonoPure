...
def multi_pose_post_process(dets, c, s, h, w):
  ...
  for i in range(dets.shape[0]):
    ...
    # 점 개수 자동 계산
    num_keypoints = (dets.shape[2] - 6) // 2  # 4 bbox + 1 score + 1 class = 6
    pts = transform_preds(
      dets[i, :, 5:5 + num_keypoints * 2].reshape(-1, 2), c[i], s[i], (w, h)
    )

    top_preds = np.concatenate(
      [bbox.reshape(-1, 4), dets[i, :, 4:5], pts.reshape(-1, num_keypoints * 2)],
      axis=1
